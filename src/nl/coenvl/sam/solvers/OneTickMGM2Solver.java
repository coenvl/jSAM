 ...
 *
 */
public class OneTickMGM2Solver implements IterativeSolver {
	...
	private static final String NO_OFFER = "MGM2:NoMoveOffer";

	private static final String ACCEPT = "MGM2:AcceptOffer";

	private static final String REJECT = "MGM2:RejectOffer";

	private static final String GAIN = "MGM2:UtilityGain";

	private static final String GO = "MGM2:GO";

	// private static final String NOGO = "MGM2:NOGO";
	...
	public OneTickMGM2Solver(LocalCommunicatingAgent agent, CostFunction costfunction) {
		this.parent = agent;
	...
	public void push(Message m) {
		...
		if (m.getType().equals(OneTickMGM2Solver.UPDATE_VALUE)) {
		...
		} else if (m.getType().equals(OneTickMGM2Solver.NO_OFFER)
				|| m.getType().equals(OneTickMGM2Solver.OFFER)) {
		...
		} else if (m.getType().equals(OneTickMGM2Solver.ACCEPT)
				|| m.getType().equals(OneTickMGM2Solver.REJECT)) {
			...
			if (m.hasContent("offer")) {
				System.out.println(this.parent.getName() + " commits to offer received from " + source.getName());
				Offer acceptedOffer = (Offer) m.getContent("offer");
				this.committedOffer = acceptedOffer;
			} 
			
			if (this.receivedAcceptRejects.size() == parent
					.getNeighborhood().size()) {
				this.receivedAcceptRejects.clear();
				this.sendGainMessages();
			}

		} else if (m.getType().equals(OneTickMGM2Solver.GAIN)) {
			Double gain = (Double) m.getContent("gain");

			this.neighborGains.put(source, gain);

			if (this.neighborGains.size() == parent.getNeighborhood().size())
				this.analyzeGains();
		} else if (m.getType().equals(OneTickMGM2Solver.GO)) {
	...
	public void tick() {
		try {
			Message updateMsg = new HashMessage(OneTickMGM2Solver.UPDATE_VALUE);
	...
	private void sendOffer() {
		...
		if (Math.random() > OneTickMGM2Solver.OFFER_PROBABILITY) {
			...
			System.out.println(this.parent.getName() + " sends offer to " + neighbor.getName());
	...
	private void analyzeOffers() {
		System.out.println(this.parent.getName() + " analyzing offers");
		Message reject = new HashMessage(OneTickMGM2Solver.REJECT);
		...
		if (!this.isOfferer) {
			...
			for (Offer suggestedOffer : this.receivedOffers) {
				...
				if (globalReduction > 0 && globalReduction > bestGain) {
					...
					bestOffer.reiceverReduction = localReduction;
				}
			}

			// Send accept if there is a global reduction
			if (bestOffer != null) {
				Message accept = new HashMessage(OneTickMGM2Solver.ACCEPT);
...
//						System.out.println(this.parent.getName() + " rejects offer from " + n.getName());
						n.push(reject);
					}
			}
		} else {
...
//				System.out.println(this.parent.getName() + " rejects offer from " + n.getName());
	...
	private void sendGainMessages() {
		...
		Message gainMessage = new HashMessage(OneTickMGM2Solver.GAIN);
		gainMessage.addContent("source", this.parent);
		if (this.committedOffer != null) {

			this.bestLocalReduction = this.committedOffer.offererReduction
	...
	private void analyzeGains() {
		...
		if (this.bestLocalReduction > bestNeighborReduction) {
			if (committedOffer == null) {
				...
				Message goMessage = new HashMessage(OneTickMGM2Solver.GO);
				...
				if (this.isOfferer) {
					...
					System.out.println(this.parent.getName() + " GO with receiver " + this.committedOffer.receiver.getName());
					this.committedOffer.receiver.push(goMessage);
				} else {
					assert(this.committedOffer.receiver == this.parent);
					System.out.println(this.parent.getName() + " GO with offerer " + this.committedOffer.receiver.getName());
					this.committedOffer.offerer.push(goMessage);
				}
			}
		} else {
			// Reset for next run. This could already be the case
			this.committedOffer = null;
		}
